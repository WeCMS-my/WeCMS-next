...
for file in "${files[@]}"; do
  ...
  if grep -q "PLACEHOLDER_RUNTIME_HASH" "$file"; then
    echo "Refusing to apply seed with runtime password hash placeholder: $file" >&2
    echo "Admin password hashes must be generated by DbMigrationRunner." >&2
    exit 1
  fi
