...
assert_contains "$captured_command_blob" '[4/18] dotnet publish (Native AOT)|run_with_dir '"$REPO_ROOT"' run_dotnet_with_cache publish backend/src/WeCms.Api/WeCms.Api.csproj -c Release -r osx-arm64 /p:PublishAot=true --nologo' \
  "Native AOT publish step should use cache-aware dotnet wrapper"
assert_contains "$captured_command_blob" '[5/18] OpenAPI export (published Native AOT binary)|run_with_dir '"$REPO_ROOT $REPO_ROOT"'/backend/src/WeCms.Api/bin/Release/net10.0/osx-arm64/publish/WeCms.Api --export-openapi ' \
  "Native AOT published binary should export OpenAPI"
assert_contains "$captured_command_blob" '[6/18] dotnet test (Unit)|run_with_dir '"$REPO_ROOT"' run_dotnet_with_cache test backend/tests/WeCms.Tests.Unit/WeCms.Tests.Unit.csproj --nologo --verbosity normal' \
  "Unit test step should use cache-aware dotnet wrapper"
assert_contains "$captured_command_blob" '[7/18] dotnet test (Integration)|run_with_dir '"$REPO_ROOT"' run_dotnet_with_cache test backend/tests/WeCms.Tests.Integration/WeCms.Tests.Integration.csproj --nologo --verbosity normal' \
  "Integration test step should use cache-aware dotnet wrapper"
assert_contains "$captured_command_blob" '[8/18] OpenAPI export|run_with_dir '"$REPO_ROOT"' run_dotnet_with_cache run --project backend/src/WeCms.Api -- --export-openapi '"$REPO_ROOT"'/artifacts/openapi/wecms-api-v1.json --nologo' \
  "OpenAPI export step should use cache-aware dotnet wrapper"
assert_contains "$captured_command_blob" '[10/18] dotnet test (Architecture)|run_with_dir '"$REPO_ROOT"' run_dotnet_with_cache test backend/tests/WeCms.Tests.Architecture/WeCms.Tests.Architecture.csproj --nologo --verbosity normal' \
