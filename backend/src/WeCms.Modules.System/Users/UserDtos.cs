 ...
 public sealed record UserListItem(long Id, string Username, string DisplayName, string? Email, string Status, bool IsSuperAdmin, DateTime CreatedAt);
 public sealed record UserDetail(long Id, string Username, string DisplayName, string? Email, string? Phone, long? AvatarFileId, string Status, bool IsSuperAdmin, bool TwoFactorEnabled, DateTime? LastLoginAt, string? LastLoginIp, DateTime CreatedAt, DateTime UpdatedAt);
