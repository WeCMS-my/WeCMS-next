...
public sealed class UserService(IPasswordHasher hasher, IDbConnectionFactory db, IClock clock, IAuditWriter audit) : IUserService
{
    ...
    { if (id == operatorId) throw new InvalidOperationException("Cannot delete yourself"); await using var c = await db.OpenAsync(ct); var isSuper = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user_role ur JOIN sys_role r ON r.id=ur.role_id WHERE ur.user_id=@Id AND r.code='super_admin' AND r.status='active' AND r.deleted_at IS NULL", new { Id = id }, cancellationToken: ct)); if (isSuper != 0) { var activeSuperCount = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(DISTINCT ur.user_id) FROM sys_user_role ur JOIN sys_role r ON r.id=ur.role_id JOIN sys_user u ON u.id=ur.user_id WHERE r.code='super_admin' AND r.status='active' AND r.deleted_at IS NULL AND u.status='active' AND u.deleted_at IS NULL", cancellationToken: ct)); if (activeSuperCount <= 1) throw new InvalidOperationException("Cannot remove the last super admin"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET deleted_at=@N, deleted_by=@Op WHERE id=@Id AND deleted_at IS NULL", new { N = clock.UtcNow.DateTime, Op = operatorId, Id = id }, cancellationToken: ct)); await audit.LogAsync("system", "user:delete", operatorId, null, null, null, 200, "success", ct); }

    public async Task SetStatusAsync(long id, string status, long operatorId, CancellationToken ct)
    { if (!AllowedStatuses.Contains(status)) throw new InvalidOperationException("Invalid status"); if (id == operatorId && status != "active") throw new InvalidOperationException("Cannot disable yourself"); await using var c = await db.OpenAsync(ct); var isSuper = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user_role ur JOIN sys_role r ON r.id=ur.role_id WHERE ur.user_id=@Id AND r.code='super_admin' AND r.status='active' AND r.deleted_at IS NULL", new { Id = id }, cancellationToken: ct)); if (isSuper != 0 && status != "active") { var activeSuperCount = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(DISTINCT ur.user_id) FROM sys_user_role ur JOIN sys_role r ON r.id=ur.role_id JOIN sys_user u ON u.id=ur.user_id WHERE r.code='super_admin' AND r.status='active' AND r.deleted_at IS NULL AND u.status='active' AND u.deleted_at IS NULL", cancellationToken: ct)); if (activeSuperCount <= 1) throw new InvalidOperationException("Cannot remove the last super admin"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET status=@S, updated_at=@N WHERE id=@Id", new { S = status, N = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); if (status != "active") { await c.ExecuteAsync(new CommandDefinition("UPDATE sys_refresh_token SET revoked_at=@N WHERE user_id=@Id AND revoked_at IS NULL", new { N = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); } await audit.LogAsync("system", "user:status", operatorId, null, null, null, 200, "success", ct); }
