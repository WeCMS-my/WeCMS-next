...
public sealed class UserService(IPasswordHasher hasher, IDbConnectionFactory db) : IUserService
{
    ...
    public async Task<long> CreateAsync(CreateUserRequest req, long operatorId, CancellationToken ct)
    {
        ...
        if (req.RoleIds is { Length: > 0 }) await conn.ExecuteAsync(new CommandDefinition("INSERT INTO sys_user_role (user_id,role_id,created_at) VALUES (@Uid,@Rid,@Now)", req.RoleIds.Select(r => new { Uid = id, Rid = r, Now = DateTime.UtcNow }), cancellationToken: ct));
        await tx.CommitAsync(ct);
        return id;
    }

    public async Task UpdateAsync(long id, UpdateUserRequest req, long operatorId, CancellationToken ct)
    { await using var c = await db.OpenAsync(ct); await using var tx = await c.BeginTransactionAsync(ct); var r = await c.QueryFirstOrDefaultAsync<UserRow>(new CommandDefinition("SELECT status, is_super_admin FROM sys_user WHERE id=@Id AND deleted_at IS NULL", new { Id = id }, cancellationToken: ct)); if (r is null) { await tx.RollbackAsync(ct); throw new InvalidOperationException("User not found"); } if (r.IsSuper != 0 && operatorId != id) { await tx.RollbackAsync(ct); throw new UnauthorizedAccessException("Cannot modify super admin"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET display_name=COALESCE(@D,display_name), email=@E, phone=@P, status=COALESCE(@S,status), row_version = row_version + 1, updated_at=@N WHERE id=@Id", new { req.DisplayName, req.Email, req.Phone, req.Status, N = DateTime.UtcNow, Id = id }, cancellationToken: ct)); if (req.RoleIds is not null) { await c.ExecuteAsync(new CommandDefinition("DELETE FROM sys_user_role WHERE user_id=@Id", new { Id = id }, cancellationToken: ct)); if (req.RoleIds.Length > 0) await c.ExecuteAsync(new CommandDefinition("INSERT INTO sys_user_role (user_id,role_id,created_at) VALUES (@Uid,@Rid,@Now)", req.RoleIds.Select(x => new { Uid = id, Rid = x, Now = DateTime.UtcNow }), cancellationToken: ct)); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET permission_version = permission_version + 1 WHERE id = @Id", new { Id = id }, cancellationToken: ct)); } await tx.CommitAsync(ct); }

    public async Task DeleteAsync(long id, long operatorId, CancellationToken ct)
    { if (id == operatorId) throw new InvalidOperationException("Cannot delete yourself"); await using var c = await db.OpenAsync(ct); var isSuper = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_super_admin FROM sys_user WHERE id=@Id", new { Id = id }, cancellationToken: ct)); if (isSuper != 0) { var activeSuperCount = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user WHERE is_super_admin=1 AND status='active' AND deleted_at IS NULL", cancellationToken: ct)); if (activeSuperCount <= 1) throw new InvalidOperationException("Cannot remove the last super admin"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET deleted_at=@N, deleted_by=@Op WHERE id=@Id AND deleted_at IS NULL", new { N = DateTime.UtcNow, Op = operatorId, Id = id }, cancellationToken: ct)); }

    public async Task SetStatusAsync(long id, string status, long operatorId, CancellationToken ct)
    { if (id == operatorId && status != "active") throw new InvalidOperationException("Cannot disable yourself"); await using var c = await db.OpenAsync(ct); var isSuper = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_super_admin FROM sys_user WHERE id=@Id", new { Id = id }, cancellationToken: ct)); if (isSuper != 0 && status != "active") { var activeSuperCount = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user WHERE is_super_admin=1 AND status='active' AND deleted_at IS NULL", cancellationToken: ct)); if (activeSuperCount <= 1) throw new InvalidOperationException("Cannot remove the last super admin"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET status=@S, row_version = row_version + 1, updated_at=@N WHERE id=@Id", new { S = status, N = DateTime.UtcNow, Id = id }, cancellationToken: ct)); }
