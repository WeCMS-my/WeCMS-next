...
public sealed class UserService(IPasswordHasher hasher, IDbConnectionFactory db) : IUserService
{
    ...
    { if (id == operatorId && status != "active") throw new InvalidOperationException("Cannot disable yourself"); await using var c = await db.OpenAsync(ct); var isSuper = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_super_admin FROM sys_user WHERE id=@Id", new { Id = id }, cancellationToken: ct)); if (isSuper != 0 && status != "active") { var activeSuperCount = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user WHERE is_super_admin=1 AND status='active' AND deleted_at IS NULL", cancellationToken: ct)); if (activeSuperCount <= 1) throw new InvalidOperationException("Cannot remove the last super admin"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET status=@S, updated_at=@N WHERE id=@Id", new { S = status, N = DateTime.UtcNow, Id = id }, cancellationToken: ct)); }
