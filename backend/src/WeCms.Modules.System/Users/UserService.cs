 ...
 public sealed class UserService(IPasswordHasher hasher, IDbConnectionFactory db)
 {
     ...
     { await using var c = await db.OpenAsync(ct); var r = await c.QueryFirstOrDefaultAsync<UserRow>(new CommandDefinition("SELECT status, is_super_admin FROM sys_user WHERE id=@Id AND deleted_at IS NULL", new { Id = id }, cancellationToken: ct)); if (r is null) throw new InvalidOperationException("User not found"); if (r.IsSuper != 0 && operatorId != id) throw new UnauthorizedAccessException("Cannot modify super admin"); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET display_name=COALESCE(@D,display_name), email=@E, phone=@P, status=COALESCE(@S,status), updated_at=@N WHERE id=@Id", new { req.DisplayName, req.Email, req.Phone, req.Status, N = DateTime.UtcNow, Id = id }, cancellationToken: ct)); if (req.RoleIds is not null) { await c.ExecuteAsync(new CommandDefinition("DELETE FROM sys_user_role WHERE user_id=@Id", new { Id = id }, cancellationToken: ct)); if (req.RoleIds.Length > 0) await c.ExecuteAsync(new CommandDefinition("INSERT INTO sys_user_role (user_id,role_id,created_at) VALUES (@Uid,@Rid,@Now)", req.RoleIds.Select(x => new { Uid = id, Rid = x, Now = DateTime.UtcNow }), cancellationToken: ct)); } }
 
     public async Task DeleteAsync(long id, long operatorId, CancellationToken ct)
     { if (id == operatorId) throw new InvalidOperationException("Cannot delete yourself"); await using var c = await db.OpenAsync(ct); if (await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_super_admin FROM sys_user WHERE id=@Id", new { Id = id }, cancellationToken: ct)) != 0) throw new UnauthorizedAccessException("Cannot delete super admin"); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET deleted_at=@N, deleted_by=@Op WHERE id=@Id AND deleted_at IS NULL", new { N = DateTime.UtcNow, Op = operatorId, Id = id }, cancellationToken: ct)); }
 
     public async Task SetStatusAsync(long id, string status, long operatorId, CancellationToken ct)
     { if (id == operatorId && status != "active") throw new InvalidOperationException("Cannot disable yourself"); await using var c = await db.OpenAsync(ct); if (await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_super_admin FROM sys_user WHERE id=@Id", new { Id = id }, cancellationToken: ct)) != 0 && status != "active") throw new UnauthorizedAccessException("Cannot disable super admin"); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_user SET status=@S, updated_at=@N WHERE id=@Id", new { S = status, N = DateTime.UtcNow, Id = id }, cancellationToken: ct)); }
