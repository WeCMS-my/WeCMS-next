 ...
 public sealed class I18nService(IDbConnectionFactory db)
 {
     public async Task<List<I18nMessageItem>> ListAsync(string? locale, string? key, CancellationToken ct)
     { await using var c = await db.OpenAsync(ct); var items = await c.QueryAsync<I18nMessageItem>(new CommandDefinition("SELECT id, locale, message_key, message_value, remark FROM sys_i18n_message WHERE (@L IS NULL OR locale=@L) AND (@K IS NULL OR message_key LIKE CONCAT('%',@K,'%')) ORDER BY locale, message_key LIMIT 500", new { L = locale, K = key }, cancellationToken: ct)); return items.AsList(); }
     ...
     { await using var c = await db.OpenAsync(ct); await c.ExecuteAsync(new CommandDefinition("DELETE FROM sys_i18n_message WHERE id=@Id", new { Id = id }, cancellationToken: ct)); }
 }
