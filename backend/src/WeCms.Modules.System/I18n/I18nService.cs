 ...
 public sealed class I18nService(IDbConnectionFactory db, IClock clock) : II18nService
 {
     ...
     { await using var c = await db.OpenAsync(ct); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_i18n_message SET message_value=COALESCE(@V,message_value), remark=COALESCE(@R,remark), updated_at=@N WHERE id=@Id", new { req.MessageValue, req.Remark, N = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); }
    public async Task DeleteAsync(long id, CancellationToken ct)
    { await using var c = await db.OpenAsync(ct); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_i18n_message SET deleted_at=@N WHERE id=@Id", new { N = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); }
 }
