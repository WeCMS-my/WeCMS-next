 ...
 public static class LogEndpoints
 {
    ...
    { var p = int.TryParse(ctx.Request.Query["page"], out var pp) ? Math.Max(pp, 1) : 1; var s = int.TryParse(ctx.Request.Query["pageSize"], out var ps) ? ps : 20; var mod = ctx.Request.Query["module"].FirstOrDefault(); var (items, total) = await svc.GetAuditLogsAsync(p, s, mod, ct); return Results.Ok(ApiResult<PagedResult<AuditLogItem>>.Ok(new PagedResult<AuditLogItem>(items, p, s, total))); }
 }
