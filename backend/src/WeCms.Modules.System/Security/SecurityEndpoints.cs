 ...
 public static class SecurityEndpoints
 {
     ...
     { var p = int.TryParse(ctx.Request.Query["page"], out var pp) ? Math.Max(pp, 1) : 1; var s = int.TryParse(ctx.Request.Query["pageSize"], out var ps) ? ps : 20; var t = ctx.Request.Query["type"].FirstOrDefault(); var (items, total) = await svc.ListEventsAsync(p, s, t, ct); return Results.Ok(ApiResult<PagedResult<SecurityEventItem>>.Ok(new PagedResult<SecurityEventItem>(items, p, s, total))); }
 }
