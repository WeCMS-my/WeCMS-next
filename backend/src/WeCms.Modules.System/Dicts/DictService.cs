 ...
 public sealed class DictService(IDbConnectionFactory db, IClock clock) : IDictService
 {
     ...
     { await using var c = await db.OpenAsync(ct); return await c.ExecuteScalarAsync<long>(new CommandDefinition("INSERT INTO sys_dict_type (code,name,status,created_at,updated_at) VALUES (@C,@N,'active',@Now,@Now); SELECT LAST_INSERT_ID();", new { C = req.Code, N = req.Name, Now = clock.UtcNow.DateTime }, cancellationToken: ct)); }
     ...
     { await using var c = await db.OpenAsync(ct); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_dict_value SET deleted_at=@Now WHERE id=@Id", new { Now = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); }
 }
