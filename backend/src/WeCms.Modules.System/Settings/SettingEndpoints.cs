 ...
 public static class SettingEndpoints
 {
     ...
     private static async Task<IResult> GetAllAsync(SettingService svc, CancellationToken ct)
         => Results.Ok(ApiResult<List<SettingItem>>.Ok(await svc.GetAllAsync(ct)));
     private static async Task<IResult> UpdateAsync(string key, UpdateSettingRequest req, SettingService svc, CancellationToken ct)
