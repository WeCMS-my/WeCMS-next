using global::WeCms.Shared;
 ...
 public static class RoleEndpoints
 {
     ...
     { var p = int.TryParse(ctx.Request.Query["page"], out var pp) ? Math.Max(pp, 1) : 1; var s = int.TryParse(ctx.Request.Query["pageSize"], out var ps) ? Math.Clamp(ps, 1, 100) : 20; var (items, total) = await svc.ListAsync(p, s, ct); return Results.Ok(ApiResult<PagedResult<RoleListItem>>.Ok(new PagedResult<RoleListItem>(items, p, s, total))); }
