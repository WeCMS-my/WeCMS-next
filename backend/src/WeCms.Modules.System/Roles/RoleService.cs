 ...
 public sealed class RoleService(IDbConnectionFactory db, IClock clock, IAuditWriter audit) : IRoleService
{
    ...
    public async Task DeleteAsync(long id, CancellationToken ct)
     { await using var c = await db.OpenAsync(ct); var isSystem = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_system FROM sys_role WHERE id=@Id", new { Id = id }, cancellationToken: ct)); if (isSystem != 0) throw new InvalidOperationException("Cannot delete system role"); var used = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user_role WHERE role_id=@Id", new { Id = id }, cancellationToken: ct)); if (used > 0) throw new InvalidOperationException("Role is assigned to users"); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_role SET deleted_at=@Now WHERE id=@Id", new { Now = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); }
