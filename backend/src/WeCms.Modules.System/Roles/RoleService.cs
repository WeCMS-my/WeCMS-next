 ...
 public sealed class RoleService(IDbConnectionFactory db) : IRoleService
 {
     ...
     { await using var c = await db.OpenAsync(ct); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_role SET name=COALESCE(@N,name), description=@D, status=COALESCE(@S,status), sort=COALESCE(@So,sort), row_version = row_version + 1, updated_at=@Now WHERE id=@Id", new { req.Name, req.Description, req.Status, So = req.Sort, Now = DateTime.UtcNow, Id = id }, cancellationToken: ct)); }
     public async Task DeleteAsync(long id, CancellationToken ct)
     { await using var c = await db.OpenAsync(ct); var isSystem = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_system FROM sys_role WHERE id=@Id", new { Id = id }, cancellationToken: ct)); if (isSystem != 0) throw new InvalidOperationException("Cannot delete system role"); var used = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user_role WHERE role_id=@Id", new { Id = id }, cancellationToken: ct)); if (used > 0) throw new InvalidOperationException("Role is assigned to users"); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_role SET deleted_at=@Now WHERE id=@Id", new { Now = DateTime.UtcNow, Id = id }, cancellationToken: ct)); }
