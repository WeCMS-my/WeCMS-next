 ...
 public sealed class RoleService(IDbConnectionFactory db, IClock clock, IAuditWriter audit) : IRoleService
{
    ...
    public async Task<(IReadOnlyList<RoleListItem> Items, long Total)> ListAsync(int page, int size, CancellationToken ct)
     ...
     { await using var c = await db.OpenAsync(ct); return await c.ExecuteScalarAsync<long>(new CommandDefinition("INSERT INTO sys_role (code,name,description,status,sort,created_at,updated_at) VALUES (@C,@N,@D,'active',@S,@Now,@Now); SELECT LAST_INSERT_ID();", new { C = req.Code, N = req.Name, req.Description, S = req.Sort, Now = clock.UtcNow.DateTime }, cancellationToken: ct)); }
     public async Task UpdateAsync(long id, UpdateRoleRequest req, CancellationToken ct)
     { await using var c = await db.OpenAsync(ct); await c.ExecuteAsync(new CommandDefinition("UPDATE sys_role SET name=COALESCE(@N,name), description=@D, status=COALESCE(@S,status), sort=COALESCE(@So,sort), updated_at=@Now WHERE id=@Id", new { req.Name, req.Description, req.Status, So = req.Sort, Now = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); }
    public async Task DeleteAsync(long id, CancellationToken ct)
     { await using var c = await db.OpenAsync(ct); await using var tx = await c.BeginTransactionAsync(ct); var isSystem = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT is_system FROM sys_role WHERE id=@Id", new { Id = id }, cancellationToken: ct)); if (isSystem != 0) { await tx.RollbackAsync(ct); throw new InvalidOperationException("Cannot delete system role"); } var used = await c.ExecuteScalarAsync<int>(new CommandDefinition("SELECT COUNT(1) FROM sys_user_role WHERE role_id=@Id", new { Id = id }, cancellationToken: ct)); if (used > 0) { await tx.RollbackAsync(ct); throw new InvalidOperationException("Role is assigned to users"); } await c.ExecuteAsync(new CommandDefinition("UPDATE sys_role SET deleted_at=@Now WHERE id=@Id", new { Now = clock.UtcNow.DateTime, Id = id }, cancellationToken: ct)); await c.ExecuteAsync(new CommandDefinition("DELETE FROM sys_role_menu WHERE role_id=@Id", new { Id = id }, cancellationToken: ct)); await c.ExecuteAsync(new CommandDefinition("DELETE FROM sys_role_permission WHERE role_id=@Id", new { Id = id }, cancellationToken: ct)); await BumpPermissionVersion(c, id, ct); await audit.LogAsync("system", "role:delete", null, null, null, null, 200, "success", ct); await tx.CommitAsync(ct); }
