...
namespace WeCms.Modules.System.Auth;
 ...
 public static class AuthManagementEndpoints
 {
     ...
     { await using var c = await db.OpenAsync(ct); var uid = await c.ExecuteScalarAsync<long?>(new CommandDefinition("SELECT id FROM sys_user WHERE email=@E AND deleted_at IS NULL AND status='active'", new { E = req.Email }, cancellationToken: ct)); if (uid.HasValue) { var token = Convert.ToHexString(RandomNumberGenerator.GetBytes(32)); var tokenHash = HashToken(token); await c.ExecuteAsync(new CommandDefinition("INSERT INTO sys_password_reset_token (user_id,token_hash,expires_at,created_at) VALUES (@U,@H,@E,@N)", new { U = uid.Value, H = tokenHash, E = DateTime.UtcNow.AddHours(1), N = DateTime.UtcNow }, cancellationToken: ct)); } return Results.Ok(ApiResult<string>.Ok("If the email exists, a reset link has been sent")); }
