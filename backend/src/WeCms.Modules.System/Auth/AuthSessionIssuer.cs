...
public sealed class AuthSessionIssuer : IAuthSessionIssuer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var menus = MenuTreeBuilder.Build(await _repository.ListVisibleMenusAsync(user.Id, user.IsSuperAdmin, cancellationToken));

        return BuildResult(user, accessToken, refreshToken, roles, permissions, menus, now);
    }
