 ...
 public static class AuthEndpoints
 {
     ...
     { var ip = ctx.Request.Headers["X-Forwarded-For"].FirstOrDefault() ?? ctx.Connection.RemoteIpAddress?.ToString() ?? "unknown"; var r = await svc.LoginAsync(req.Username, req.Password, ip, ct); return r is not null ? Results.Ok(ApiResult<LoginResponse>.Ok(r)) : Results.Ok(ApiResult<LoginResponse>.Fail(ApiCodes.Unauthorized, "Invalid credentials")); }
     ...
     { var rt = ctx.Request.Headers["X-Refresh-Token"].FirstOrDefault(); if (!string.IsNullOrEmpty(rt)) await svc.LogoutAsync(rt, ct); return Results.Ok(ApiResult<string>.Ok("logged out")); }
