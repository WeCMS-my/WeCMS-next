 ...
 public static class AuthEndpoints
 {
     ...
     { var rt = ctx.Request.Headers["X-Refresh-Token"].FirstOrDefault(); if (string.IsNullOrEmpty(rt)) { ctx.Response.StatusCode = 400; return Results.Ok(ApiResult<string>.Fail(ApiCodes.ValidationError, "X-Refresh-Token header required")); } await svc.LogoutAsync(rt, ct); return Results.Ok(ApiResult<string>.Ok("logged out")); }
