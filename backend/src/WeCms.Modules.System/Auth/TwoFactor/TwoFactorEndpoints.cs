 ...
 public static class TwoFactorEndpoints
 {
     ...
     private static async Task<IResult> VerifyAsync(TwoFactorVerifyRequest req, ITwoFactorService twoFactor, IDbConnectionFactory db, CancellationToken ct)
     { if (string.IsNullOrWhiteSpace(req.Username) || string.IsNullOrWhiteSpace(req.Code)) return Results.Ok(ApiResult<string>.Fail(ApiCodes.ValidationError, "Username and code required")); await using var c = await db.OpenAsync(ct); var secret = await c.QueryFirstOrDefaultAsync<string>(new CommandDefinition("SELECT two_factor_secret FROM sys_user WHERE username=@U AND deleted_at IS NULL AND two_factor_enabled=1", new { U = req.Username }, cancellationToken: ct)); if (string.IsNullOrEmpty(secret)) return Results.Ok(ApiResult<string>.Fail(ApiCodes.BusinessError, "2FA not configured")); var ok = twoFactor.Verify(secret, req.Code); return ok ? Results.Ok(ApiResult<string>.Ok("ok")) : Results.Ok(ApiResult<string>.Fail(ApiCodes.BusinessError, "Invalid code")); }
