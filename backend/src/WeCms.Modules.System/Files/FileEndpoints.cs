 ...
 public static class FileEndpoints
 {
     ...
     { var p = int.TryParse(ctx.Request.Query["page"], out var pp) ? Math.Max(pp, 1) : 1; var s = int.TryParse(ctx.Request.Query["pageSize"], out var ps) ? ps : 20; var (items, total) = await svc.ListAsync(p, s, ct); return Results.Ok(ApiResult<PagedResult<FileItem>>.Ok(new PagedResult<FileItem>(items, p, s, total))); }
