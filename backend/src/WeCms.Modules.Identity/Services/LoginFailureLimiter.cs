...
public sealed class LoginFailureLimiter : ILoginFailureLimiter
{
    ...
    private readonly IIdentitySecurityBanService _securityBanService;
    ...
    public LoginFailureLimiter(
        ...
        IIdentitySecurityAlertService alertService,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (context.UserId is { } userId)
        {
            await CreateIfNoActiveBanAsync(
                new IdentitySecurityBanCreateRecord(
                    IdentitySecurityBanTypes.User,
    ...
    private async Task CreateIfNoActiveBanAsync(
        ...
        CancellationToken cancellationToken)
    {
        var active = await _securityBanService.HasActiveAsync(record.BanType, record.Target, record.CreatedAt, cancellationToken);
        if (!active)
        {
