...
public sealed class MenuService : IMenuService
{
    ...
    private readonly IAccessControlPermissionVersionService _permissionVersionService;
    ...
    public MenuService(
        ...
        IAccessControlPermissionVersionService permissionVersionService,
    ...
    public async Task<MenuMutationResponse> CreateAsync(CreateMenuRequest request, MenuRequestContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await WriteMenuChangedAsync(context, [id], cancellationToken);
    ...
    public async Task SortAsync(SortMenusRequest request, MenuRequestContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await WriteMenuChangedAsync(context, records.Select(record => record.Id).ToArray(), cancellationToken);
            await transaction.CommitAsync(cancellationToken);
        }
        catch
        {
    ...
    private Task WriteMenuChangedAsync(MenuRequestContext context, IReadOnlyList<long> menuIds, CancellationToken cancellationToken)
    {
        return _outboxWriter.WriteAsync(
            new MenuChangedEvent(NewEventId(), context.Now, context.TraceId, null, menuIds),
            cancellationToken);
    }
