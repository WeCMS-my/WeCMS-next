...
namespace WeCms.Modules.Configuration.SqlSugar.Repositories;
...
public sealed class DictRepository : IDictRepository
{
    ...
    public async Task<long> CreateTypeAsync(DictTypeCreateRecord record, CancellationToken cancellationToken)
    {
        ...
        return Convert.ToInt64(await _db.Ado.GetScalarAsync(
            "SELECT id FROM sys_dict_type WHERE code = @code AND deleted_at IS NULL LIMIT 1",
            new SugarParameter("@code", record.Code)), global::System.Globalization.CultureInfo.InvariantCulture);
    }
    ...
    public async Task<long> CreateValueAsync(DictValueCreateRecord record, CancellationToken cancellationToken)
    {
        ...
        return Convert.ToInt64(await _db.Ado.GetScalarAsync(
            "SELECT id FROM sys_dict_value WHERE type_id = @typeId AND value = @value AND deleted_at IS NULL LIMIT 1",
            new SugarParameter("@typeId", record.TypeId),
            new SugarParameter("@value", record.Value)), global::System.Globalization.CultureInfo.InvariantCulture);
    }
