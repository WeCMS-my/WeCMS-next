...
{
    public static IEndpointRouteBuilder MapSecurityManagementEndpoints(this IEndpointRouteBuilder endpoints)
    {
        var group = endpoints.MapGroup("/api/v1/system/security")
            .WithEndpointModule("security")
            .AuditWriteEndpoints("security", "security-bans")
        ...
        group.MapPost("/bans/batch-unban", BatchUnbanAsync).RequireEndpointPermission(SecurityPermissions.BanBatchUnban).RequireRateLimiting(RateLimitPolicyNames.SecurityUnban);
    ...
    private static SecurityBanRequestContext Context(HttpContext httpContext, ISecurityClock clock)
    {
