...
public sealed class DictService : IDictService
{
    ...
    public async Task<DictMutationResponse> CreateTypeAsync(CreateDictTypeRequest request, DictRequestContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            id = await _repository.CreateTypeAsync(new DictTypeCreateRecord(code, name, description, request.SortOrder, status, context.Now), cancellationToken);
            await AuditAsync(context, "create-type", id, "dict-type", "success", "Dictionary type created.", cancellationToken);
            await WriteChangedAsync(context, "dict-type", id, cancellationToken);
        ...
        await InvalidateDictsAsync(cancellationToken);
    ...
    public async Task<DictMutationResponse> CreateValueAsync(string typeCode, CreateDictValueRequest request, DictRequestContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            id = await _repository.CreateValueAsync(new DictValueCreateRecord(type.Id, label, value, description, request.SortOrder, request.IsDefault, status, context.Now), cancellationToken);
            await AuditAsync(context, "create-value", id, "dict-value", "success", "Dictionary value created.", cancellationToken);
            await WriteChangedAsync(context, "dict-value", id, cancellationToken);
            await transaction.CommitAsync(cancellationToken);
        }
        catch
        {
    ...
    private Task WriteChangedAsync(DictRequestContext context, string resource, long targetId, CancellationToken cancellationToken)
    {
        return _outboxWriter.WriteAsync(new DictChangedEvent(NewEventId(), context.Now, context.TraceId, null, resource, targetId), cancellationToken);
    }

    private Guid NewEventId() => Guid.ParseExact(_idGenerator.NewId(), "N");

    private Task InvalidateDictsAsync(CancellationToken cancellationToken) => _cacheInvalidator.InvalidateDictsAsync(cancellationToken);
