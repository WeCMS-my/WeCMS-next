...
public sealed class AuditLogApiScanTests
{
    ...
    {
        var source = File.ReadAllText(Path.Combine(TestPaths.RepoRoot, "backend", "src", "WeCms.Modules.Audit", "Logs", "AuditLogEndpoints.cs"));
        ...
        Assert.Contains("RequireEndpointPermission(LogPermissions.AuditLogList)", source, StringComparison.Ordinal);
        Assert.Contains("RequireEndpointPermission(LogPermissions.AuditLogDetail)", source, StringComparison.Ordinal);
    }
}
