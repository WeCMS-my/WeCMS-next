...
public sealed class DatabaseGovernanceSourceTests
{
    ...
    {
        var source = await File.ReadAllTextAsync(RepoPath("backend", "tests", "WeCms.Tests.Integration", "IntegrationTestDatabase.cs"));
    ...
    {
        var source = await File.ReadAllTextAsync(RepoPath("backend", "src", "WeCms.Api", "Extensions", "DatabaseMigrationCommand.cs"));
    ...
    {
        var source = await File.ReadAllTextAsync(RepoPath("backend", "src", "WeCms.Persistence", "Data", "PersistenceServiceCollectionExtensions.cs"));
