...
| `FileStorage:VirusScanEnabled` | Must be `true` in Production. Development may keep it disabled. |
| `FileStorage:VirusScan:Provider` | Must be `clamav-tcp` in Production. |
| `FileStorage:VirusScan:Host` | Required scanner host in Production. Use an internal service name such as `scanner.internal`. |
...
- `NoopFileScanService` is allowed only outside Production when virus scanning is disabled.
...
- Deploy and validate a reachable ClamAV TCP scanner before starting Production.
- Set `FileStorage:VirusScanEnabled=true`, `FileStorage:VirusScan:Provider=clamav-tcp`, and `FileStorage:VirusScan:Host`. Override `Port`, `TimeoutSeconds`, and `ChunkSizeBytes` only when the defaults do not match the deployed scanner.
