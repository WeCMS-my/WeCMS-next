...
> 生成日期：2026-06-07  
> 适用阶段：M0 工程启动阶段  
> 技术栈：ASP.NET Core Minimal APIs、.NET 10、Native AOT Only、SqlSugar ORM、MySQL、SoybeanAdmin  
...
```text
M0：工程骨架搭建 + AOT 可发布验证 + SqlSugar ORM 数据访问验证 + OpenAPI 契约闭环 + SoybeanAdmin 联通验证
```
...
| `backend/src/WeCms.Persistence` | 数据库连接、migration runner、SqlSugar ORM repository 实现；允许引用 `WeCms.Modules.System` / `WeCms.Modules.Cms` 仅用于实现模块暴露的 repository port |
...
| M0-05 | SqlSugar ORM 查询样例 | 强类型查询成功，无 dynamic |
...
### 7.4 SqlSugar ORM 数据访问规则

M0 即执行以下限制：

```text
1. 禁止 dynamic 查询和 dynamic 返回。
2. 禁止 SELECT *。
3. 禁止拼接前端参数到 SQL。
4. 排序字段必须白名单。
5. Repository 返回强类型 DTO。
6. Repository 方法必须接收 CancellationToken。
7. 写操作必须校验 affected rows 或等效执行结果。
8. 原始 SQL 必须进入 Code Review。
9. SQL、SqlSugar ORM、MySqlConnector 只能出现在 WeCms.Persistence。
...
### 7.6 SqlSugar ORM 验收项

```text
[ ] AOT publish 无 SqlSugar ORM 相关阻断。
...
M0 每个开发任务完成后，必须先运行该任务对应测试和门禁。只有测试和门禁全部通过，才允许进入下一项任务。

```text
1. 当前任务测试未运行或未通过：不得进入下一项任务。
2. 当前任务门禁未运行或未通过：不得进入下一项任务。
3. 阶段门禁脚本尚未创建时，使用该任务验收命令和当前已存在的等效检查作为临时门禁。
...
| M0-DB-003 | SqlSugar ORM 查询 | Repository | 强类型查询成功 |
...
| SqlSugar ORM 限制 | dynamic、复杂映射不可用 | Repository 只用强类型 DTO |
