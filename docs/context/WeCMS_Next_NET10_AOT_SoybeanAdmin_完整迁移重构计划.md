...
```text
CI-TASK-001：每个开发任务完成后，必须运行该任务对应测试。
CI-TASK-002：每个开发任务完成后，必须运行该任务对应质量门禁。
CI-TASK-003：测试和质量门禁全部通过后，才允许进入下一项任务。
CI-TASK-004：测试或质量门禁失败时，必须停留在当前任务修复，禁止推进后续任务。
CI-TASK-005：阶段门禁脚本尚未创建时，必须使用当前阶段文档规定的等效验收命令作为临时门禁。
```
