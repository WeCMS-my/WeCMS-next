...
> 目标技术栈：ASP.NET Core Minimal APIs / .NET 10 / Native AOT Only / Dapper / Dapper.AOT / SoybeanAdmin  
...
新系统：ASP.NET Core Minimal APIs + .NET 10 Native AOT + Dapper.AOT + SoybeanAdmin
```
...
6. 数据访问采用 Dapper + Dapper.AOT，不使用 EF Core。
...
Application Services
  ↓
Dapper / Dapper.AOT Repositories
  ↓
MySQL 8 / MySQL 8.4
```
...
<Project Sdk="Microsoft.NET.Sdk.Web">
  ...
  <ItemGroup>
    ...
    <PackageReference Include="Dapper.AOT" Version="*" />
...
2. 不使用 dynamic。
3. 不使用 SELECT *。
4. SQL 列名与 DTO 属性显式匹配。
5. 参数对象使用明确匿名对象或参数类型。
6. 所有分页 SQL 必须有限制。
7. 写操作必须使用事务。
8. 所有 SQL 必须可单元测试或迁移测试覆盖。
```
    ...
    CancellationToken cancellationToken)
{
    ...
    var rows = await connection.QueryAsync<UserListItem>(
        new CommandDefinition(
            sql,
            new { Keyword = keyword, Offset = offset, Limit = limit },
            cancellationToken: cancellationToken));
...
```csharp
// 禁止
await connection.QueryAsync<dynamic>("select * from sys_user");
...
// 禁止
await connection.QueryAsync(typeFromRuntime, sql);
```
...
4. 配置 Dapper / Dapper.AOT / MySqlConnector。
5. 配置 API Response。
...
| Dapper.AOT 支持范围限制 | 高 | 禁用 dynamic，SQL 显式化，建立查询规范 |
...
4. 所有 SQL 使用 Dapper / Dapper.AOT 规范。
...
### 27.8 SQL、Dapper、Dapper.AOT 强制规则

```text
DAPPER-001：禁止 Query<dynamic>。
DAPPER-002：禁止 SELECT *。
DAPPER-003：禁止拼接 SQL 参数。
DAPPER-004：排序字段必须白名单映射。
DAPPER-005：分页参数必须后端校验。
DAPPER-006：所有 SQL 参数必须命名参数化。
DAPPER-007：Repository 只允许返回 DTO / Record，不返回 dynamic。
DAPPER-008：事务必须由 Service 层控制。
DAPPER-009：Repository 不得直接提交业务事务。
DAPPER-010：所有写 SQL 必须返回影响行数并校验。
DAPPER-011：删除必须默认软删除。
DAPPER-012：批量操作必须限制最大数量。
DAPPER-013：所有迁移 SQL 必须可重复演练。
DAPPER-014：所有 Repository 方法必须接收 CancellationToken。
DAPPER-015：所有 SQL CommandTimeout 必须显式配置。
DAPPER-016：禁止循环中执行 N+1 查询。
DAPPER-017：禁止无 where 条件更新或删除。
DAPPER-018：所有慢 SQL 必须进入性能日志。
```
...
Dapper 项目高度依赖 SQL 纪律。所有数据库变更必须版本化、可审计、可演练。
...
### 28.21 Dapper.AOT 深度约束规则

Dapper.AOT 不支持所有 Dapper 动态能力，因此必须从编码规范上规避不兼容模式。

```text
DAPPER-AOT-PLUS-001：所有查询方法必须使用具体 DTO 泛型。
DAPPER-AOT-PLUS-002：禁止使用 dynamic 参数和 dynamic 返回。
DAPPER-AOT-PLUS-003：禁止运行时决定返回类型。
DAPPER-AOT-PLUS-004：禁止多映射复杂魔法查询进入核心路径。
DAPPER-AOT-PLUS-005：多表聚合优先使用显式 DTO。
DAPPER-AOT-PLUS-006：所有 SQL 文件或 SQL 字符串必须纳入 Review。
DAPPER-AOT-PLUS-007：SQL 变更必须有 explain 或索引评估。
DAPPER-AOT-PLUS-008：Repository 方法必须有集成测试覆盖。
DAPPER-AOT-PLUS-009：Repository 方法必须验证 AOT publish。
DAPPER-AOT-PLUS-010：Dapper.AOT warning 不允许忽略。
DAPPER-AOT-PLUS-011：禁止非泛型传入运行时 Type 的查询模式。
DAPPER-AOT-PLUS-012：查询 DTO 字段必须与 SQL alias 明确对应。
```
...
ACCEPT-ARCH-004：Dapper.AOT 约束通过。
...
前面章节已经完成架构、AOT、Dapper.AOT、后端契约优先、安全、WAF、性能、CI/CD、高级治理和基础系统能力闭环设计。本章进一步把这些规则转换为**可开发、可测试、可迁移、可验收、可交接的工程交付工件**。
...
##### sys_setting_group / sys_setting / sys_setting_change_log
...
| `value_type` | `varchar(32)` | string/int/bool/json/secret_ref |
...
##### sys_login_log / sys_audit_log / sys_security_event
...
##### sys_dept / sys_post / sys_user_dept / sys_role_dept
...
##### sys_notice / sys_message / sys_message_receiver

用于公告、站内信和通知。

##### sys_mail_template / sys_mail_outbox
...
##### cms_content_revision / cms_content_publish_log
...
| 旧表 | 新表 | 规则 |
|---|---|---|
| `think_config` | `sys_setting` | 敏感配置转为 secret_ref 或加密值 |
...
| `ROLE-001` | 角色 | 修改角色权限 | permission_version 更新 |
...
- Dapper.AOT GitHub：`https://github.com/DapperLib/DapperAOT`
- Dapper.AOT 文档：`https://aot.dapperlib.dev/`
- Dapper.AOT NuGet：`https://www.nuget.org/packages/Dapper.AOT`
