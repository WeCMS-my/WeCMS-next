...
```text
TASK-GATE-001：每个开发任务完成后，必须运行该任务对应测试。
TASK-GATE-002：每个开发任务完成后，必须运行该任务对应质量门禁。
TASK-GATE-003：测试和质量门禁全部通过后，才允许进入下一项任务。
TASK-GATE-004：测试或质量门禁失败时，必须停留在当前任务修复，禁止推进后续任务。
TASK-GATE-005：阶段门禁脚本尚未创建时，必须使用当前阶段文档规定的等效验收命令作为临时门禁。
```
